...
→ 주기성이 보이는 파형. artifact 때문이 아닌 순수 이상치임을 확인

## 3. 수정 목표

**초기 목표**

지도 학습 기반의 EEG 신호 단순 분류 모델 설계. 주어진 EEG 데이터를 바탕으로 seizure, LPD, GPD, LRDA, GRDA, Other와 같은 클래스를 예측하는 분류 모델을 구축하는 것.

**수정된 목표**

artifact 제거를 포함한 신뢰 가능한 EEG 이상 탐지 시스템 구축. 단순히 EEG를 정해진 클래스로 분류하는 것이 아니라, 노이즈와 artifact의 영향을 줄이고 비지도 학습 기반으로 의미 있는 EEG 패턴을 탐지하는 방향으로 목표를 수정함.

**수정 이유**

EEG 데이터는 생체 신호 특성상 노이즈와 artifact가 많이 포함되어 있음. 이를 고려하지 않고 단순 분류 모델을 학습할 경우 모델이 실제 뇌파 이상 패턴이 아니라 잡음이나 artifact에 반응할 가능성이 있음. 따라서 본 프로젝트는 지도 학습 기반 단순 분류에서 벗어나, EEG 신호의 통계적 특징을 활용해 더 신뢰 가능한 이상 패턴을 탐지하는 비지도 학습 기반 분석으로 방향을 수정.
...
- anomaly vs normal feature 비교

- 이상 신호의 통계적 특징 정리

### 4.2 클러스터 해석

- cluster별 feature 평균 분석

- EEG 패턴별 특징 정의

### 4.3 이상 신호 조기 감지 모델 설계 
